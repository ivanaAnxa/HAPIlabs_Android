...
public class JsonGetWeightDataResponseHandler extends JsonDefaultResponseHandler {
    ...
    public JsonGetWeightDataResponseHandler(Handler handler) {
        super(handler);
...
//            JSONObject api_response = jsonResponse.getJSONObject("message_detail");
            ...
            if (status == null || status.equalsIgnoreCase("Failed")) {
                ...
                JSONObject json = jsonResponse.getJSONObject("weight");
                Weight weight = JsonUtil.getWeightView(json);

                //transfer the message obj to the response handler
                setResponseObj(weight);
