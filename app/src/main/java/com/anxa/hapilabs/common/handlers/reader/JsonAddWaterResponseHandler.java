...
/**
 * Created by aprilanxa on 26/07/2016.
 */
public class JsonAddWaterResponseHandler extends JsonDefaultResponseHandler {
    ...
    public JsonAddWaterResponseHandler(Handler handler) {
        super(handler);
        this.handler = handler;

    }

    public JsonAddWaterResponseHandler(Handler handler, String strJson) {
        super(handler);
        this.handler = handler;
    ...
    public void start() {
        ...
        try {
            ...
            System.out.println("JsonAddWaterResponseHandler ");

            msg.what = JsonDefaultResponseHandler.START;
            handler.handleMessage(msg);

            jsonResponse = new JSONObject(strJson);
            System.out.println("JsonAddWaterResponseHandler " + strJson);
            ...
            if (jsonResponse.has("api_response")) {
                ...
                /** {"api_response":
                 * {"status":"Successful"},
                 * "water":{"number_glasses":5,"water_datetime":"1468741379","water_id":"87169743"}}**/
                ...
                if (requestStatus.equalsIgnoreCase("Successful")) {
                    // if success
                    JSONObject messageObj = jsonResponse.getJSONObject("water");

                    ApplicationEx.getInstance().currentWater = JsonUtil.getWater(messageObj);
                    ApplicationEx.getInstance().waterList.add(JsonUtil.getWater(messageObj));
