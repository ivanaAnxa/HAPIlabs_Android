 ...
 * request
 */
public class JsonSessionTrackingResponseHandler extends
		JsonDefaultResponseHandler {
	 ...
	 * 
	 * 
	 * {"error":null,"error_count":0,"message":"Successful","message_detail":
	 * "Logged successfully."}
	 * 
	 * 
	 * */

	public JsonSessionTrackingResponseHandler(Handler handler, String strJson) {
		super(handler);
		 ...
		 */
		if (strJson.isEmpty()) {
			strJson = "{\"error\":null,\"error_count\":0,\"message\":\"Successful\",\"message_detail\":\"Logged successfully.\"}";
		}
	...
	public void start() {
		...
		try {
			...
			// if success
			MessageObj msgObj = JsonUtil.getMessageObj(MESSAGE_TYPE.SUCCESS, jsonResponse);
			setResponseObj(msgObj);
			msg.what = JsonDefaultResponseHandler.COMPLETED;
			handler.handleMessage(msg);
			return;
